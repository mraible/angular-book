...
[IMPORTANT]
====
The `principal.name` will not resolve to the user's email address by default. To fix this, you need to add an Action in Auth0 that will add the email address to the access token. Log in to your Auth0 management dashboard and go to *Actions* > *Library* > *Build Custom*.
...
exports.onExecutePostLogin = async (event, api) => {
  const namespace = 'https://angular-book.com';
...
Now, when you log in, the email address will be added to the access token.
