...
After creating your app, click on the **Resources** tab and add the **Auth0** add-on.

// todo: update for Auth0 add-on https://devcenter.heroku.com/articles/auth0

.Auth0 Add-On
image::angular-deployment/okta-add-on.png[Okta Add-On, 450, scaledwidth="55%", align=center]
...
Click **Submit Order Form** and wait 20-30 seconds while your Auth0 account is created and OIDC apps are registered. Now, go to your app's **Settings** tab and click the **Reveal Config Vars** button. The Config Vars displayed are the environment variables you can use to configure both Angular and Spring Boot for OIDC authentication.

.Okta Add-On
image::angular-deployment/heroku-config-vars.png[Okta Add-On, 800, scaledwidth="100%", align=center]

.What if I already have an Auth0 account?
****
If you already have an Auth0 account, you can use it too. However, the Auth0 add-on makes things easier because it creates SPA and Web OIDC apps on Auth0 for you. It automatically configures their redirect URIs for Heroku too.

If you want to use an existing Auth0 account, I recommend using the https://github.com/auth0/auth0-cli[Auth0 CLI]. The CLI makes it easy to create similar OIDC apps with `auth0 apps create`. You will need to make sure their redirect URIs match your Heroku app.
****

Update the `.okta.env` file in the `angular-deployment/notes-api` directory and copy the config vars into it, where `$AUTH0_*` is the value from Heroku.

// heroku config -s | grep 'AUTH0_CLIENT_ID\|AUTH0_CLIENT_SECRET\|AUTH0_DOMAIN' | tee -a .okta.env

[source,shell]
----
export OKTA_OAUTH2_ISSUER=$AUTH0_DOMAIN
export OKTA_OAUTH2_CLIENT_ID=$AUTH0_CLIENT_ID
export OKTA_OAUTH2_CLIENT_SECRET=$AUTH0_CLIENT_SECRET
----

[NOTE]
====
If you're on Windows without https://docs.microsoft.com/en-us/windows/wsl/install[Windows Subsystem for Linux] installed, create an `okta.bat` file and use `SET` instead of `export`.
====
...
Next, configure Angular for OIDC authentication by modifying its `auth-routing.module.ts` to use the generated domain and client ID.
...
CAUTION: If you get an error when running `npm i`, delete `node_modules` and `package-lock.json` and try again.
...
Click the **Login** button in the top right corner. You should be logged in straight away since you're already logged in to Auth0. If you want to see the full authentication flow, log out, or try it in a private window. You can use the `$AUTH0_ADMIN_EMAIL` and `$AUTH0_ADMIN_PASSWORD` from your Heroku config variables for credentials. Create a note to make sure everything works.

// todo: verify the Auth0 add-on provides credentials as environment variables
...
At the time of this writing, `ncu -u` will upgrade Angular from version ~13.2.0 to ~13.2.3 and TypeScript to version ~4.5.5.
...
Now, when you run `ng build` to create a production build, it will replace the values in `environment.ts` with those in `environment.prod.ts`.

// todo: verify `--configuration prod` is not needed for `build` script
...
You should already have a Heroku app that you added Auth0 to. Let's use it for hosting Spring Boot. Run `heroku apps` and you'll see the one you created.
...
The `$AUTH0_*` environment variables don't have the same names as the Okta Spring Boot starter expects. This is because the Auth0 Heroku Add-On creates two apps: SPA and web. The web app's config variables end in `_WEB`. You'll have to make some changes so those variables are used for the Okta Spring Boot starter. One way to do so is to create a `Procfile` in the `notes-api` directory.

[source,shell]
----
web: java -Dserver.port=$PORT -Dokta.oauth2.client-id=${AUTH0_CLIENT_ID} -Dokta.oauth2.client-secret=${AUTH0_CLIENT_SECRET} -jar build/lib/*.jar
----

// todo: verify the following paragraph is still true
...
.Hello, SUPER ADMIN
image::angular-deployment/heroku-hello.png[Hello, SUPER ADMIN, 800, scaledwidth="100%", align=center]
...
You won't be able to log in to your app until you modify its Sign-in redirect URI on Auth0. Log in to your Auth0 dashboard. (tip: you can do this from the first Heroku app you created, under the **Resources** tab by clicking on **Auth0**.) Go to the **Applications** section, find your **SPA** app, and edit it. Add `https://<angular-app-on-heroku>.herokuapp.com/callback` to the Sign-in redirect URIs and `https://<angular-app-on-heroku>.herokuapp.com` to the Sign-out redirect URIs.
...
// todo: update for NGINX buildpack
