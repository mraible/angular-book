...
Before you start creating an Angular app, I think it's useful to know a bit about the history of Angular. It's first version was called AngularJS. Then, from its second version until today, it's been called Angular.
...
If you don't have the Angular CLI installed, install it:
...
The test below should be on the same level as `beforeEach`. Passing the `done` function into the test ensures the test doesn't complete and exit prior to all the asserts running.
