...
Because there won't be any cross-domain requests, you can remove the `corsConfigurationSource` bean and `allowedOrigins` variable in `SecurityConfiguration.kt`, too.
...
// todo: is this still necessary or did they change the defaults in Spring Boot 3?
...
- Web origin: `\https://<heroku-app>.herokuapp.com`
...
metadata:
  ...
  labels:
    serving.knative.dev/release: v1.9.3
...
OKTA_AUDIENCE=<your-auth0-client-secret>
----
...
----
NAME               URL
bootiful-angular   https://bootiful-angular.default.34.171.210.214.sslip.io
----

// todo: why is the current result http instead of https?
...
- Callback: `\https://bootiful-angular.default.34.171.210.214.sslip.io/login/oauth2/code/okta`
- Logout: `\https://bootiful-angular.default.34.171.210.214.sslip.io`

Then, you'll be able to sign in to your app running on Knative! Add a note or two to prove it all works.

// todo: update image when it works
