...
export class AuthService {
  ...
  logout(): void { // <.>
    ...
    this.http.post(`${environment.apiUrl}/api/logout`, {}).subscribe((response: any) => {
      location.href = response.logoutUrl + '?client_id=' + response.idToken + '&returnTo=' + redirectUri;
    });
  }
}
----
...
<.> Logout using the `/api/logout` endpoint, which returns the Auth0 Logout API URL and the client ID token.
====
...
In the Spring Boot app, you'll need to adjust how the email is retrieved from the user's profile since the authentication object returned from Spring Security will be an `OAuth2AuthenticationToken` instead of a `JwtAuthenticationToken`. This is because it's no longer acting as a resource server and can get user information from the authorization server.

In `DemoApplication.kt`, change the type of token that's imported.
...
Then, modify its `AddUserToNote` class to get the email from the new token type.
...
class AddUserToNote {
    ...
    fun handleCreate(note: Note) {
        ...
        val email = (auth as OAuth2AuthenticationToken).principal.attributes["email"] as String
        note.username = email
...
You'll need to make the same change to the import and `email` variable in `UserController.kt`.
...
class UserController(val repository: NotesRepository) {
    ...
    fun notes(principal: Principal, title: String?, pageable: Pageable): Page<Note> {
        val email = (principal as OAuth2AuthenticationToken).principal.attributes["email"] as String
        ...
    }
 ...
 * This solution is <a href="https://github.com/spring-projects/spring-security/issues/12141#issuecomment-1321345077">recommended</a> by Spring Security.
 */
...
todo: Should a bit.ly link be used in the Javadoc above? https://bit.ly/3MDwjnj
