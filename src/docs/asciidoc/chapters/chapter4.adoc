...
While you're in there, change RxJS to use version `~6.6.0`.
...
Use the following command with your values
...
Near the bottom, enter `index.html` as the **Default Root Object**, and select **Create Distribution**. When the Last modified column changes from `Deploying` to today's date, navigate to the domain name in your browser.
