...
[big red yellow-background]*TODO: <Deepu:Suggestion>: What about the javaScript for search function? Its a bit different from last chapter*
...
////
Error: src/app/note/note-list/note-list.component.html:45:30 - error TS2322: Type 'number | null' is not assignable to type 'number'.
Type 'null' is not assignable to type 'number'.

45                             [collectionSize]="total$ | async" [(page)]="filter.page" [pageSize]="filter.size"
~~~~~~~~~~~~~~

  src/app/note/note-list/note-list.component.ts:10:16
    10   templateUrl: 'note-list.component.html'
                      ~~~~~~~~~~~~~~~~~~~~~~~~~~
    Error occurs in the template of component NoteListComponent.
////
...
TIP: If you want to be more strict about your typing, you can make the `total$` Observable use `<number | null>` as its type. I used `<any>` as a shortcut.
...
[big red yellow-background]*TODO: <Deepu:Suggestion>: Can't we use `NoteFilter` instead of `any` for params?*
