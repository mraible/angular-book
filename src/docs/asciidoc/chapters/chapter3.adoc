...
[big red yellow-background]*TODO: <Deepu:Suggestion>: Might be nice to mention that BS was different from other CSS frameworks at the time due to the fact that they embraced mobile first design and made responsive design the norm for web design. Before BS we were building different UIs for desktop and mobile using stuff like jQuery mobile and so on*
...
[big red yellow-background]*TODO: <Deepu:Suggestion>: Might be nice to mention that the styling on validation comes from the code added to [ngClass], especially nice for beginners*
...
[big red yellow-background]*TODO: <Deepu:Suggestion>: Might be useful to mention what "total$ | async" does, I had to look it up as it seems new in angular (or may be I havent done angular in a while :P)*
